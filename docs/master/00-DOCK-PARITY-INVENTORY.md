...
| D13 | Safari page actions | ✅ | Tabs, **exactly like Global Context's running apps** (owner 2026-09-25): a Safari scope uses Global's shell and height; at rest it folds into the dock bar with the open tabs in the running apps' place (pins and tools kept at the end); the field carries them as its small pill, "+N" past Global's pill capacity; hovering the pill folds back to the bar; clicking a tab (big or small) switches Safari. From the Dock's loader (`SafariTabManager`) through the shared `BrowserTabList` (order, URL key, favicon) the Dock's strip uses too. The hint says "tabs" only for Safari; Chrome/Arc list none in either shell. Checked on the app 2026-09-25. `CornerSafariTabsTests` (`theSafariScopeUsesTheGlobalShell`, `theTabsAreTheStripIcons`, `overflowGoesToPlusN`, `aTabSwitches`, `currentTabFirst`, `onlySafariShowsTabs`, `theHintMatchesWhatIsShown`) |
