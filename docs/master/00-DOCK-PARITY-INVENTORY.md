...
| D13 | Safari page actions | ✅ | Tabs: in the Corner's Safari scope the open tabs are **pills beside the field**, like the Dock's tab strip — current page first; the field grows to hold them (its chip and buttons counted), the rest "+N" past the screen's budget; typing narrows them; clicking one switches Safari and leaves the Corner up. From the Dock's loader (`SafariTabManager`) through the shared `BrowserTabList` (order, filter, URL key, favicon) the Dock's strip now uses too. The hint says "tabs" only for Safari; Chrome/Arc list none in either shell. Checked on the app 2026-09-25 (3, 9, 15, 27 tabs). `CornerSafariTabsTests` (`theSafariScopeShowsTabPills`, `overflowGoesToPlusN`, `typingFiltersTabs`, `aTabPillSwitches`, `currentTabFirst`, `onlySafariShowsTabs`, `theHintMatchesWhatIsShown`) |
