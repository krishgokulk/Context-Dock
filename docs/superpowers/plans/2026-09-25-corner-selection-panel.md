...
> **Status: BUILDING (2026-09-25). Part A ✅ (checked on the app). A2 ✅ (a64e5a1; full suite 1949/1949). B ✅ route 1 (native destinations in the card, Share on answers); route 3 (typed "send to …") parked — see B note. B checked on the app (TextEdit: Share Selection → destinations with icons, typing narrows, Esc → actions → closed; a double-Esc bug found and fixed). Route 3 ✅ (sent live to the owner). **Row-by-row check on the app (TextEdit), in progress:** ✅ Copy Text, Copy as Plain Text (asks Run in the card), Save Selection to Downloads, Search on YouTube, Search Web, Summarize, Rewrite, Make table (table renders), Share list, send-to. Bugs found and fixed: send-to hijacked "copy text"; extension alert unreachable; empty scriptPath wiped scripts; selection questions routed to rules/extensions; answer closed after 8 s / went blank. **Still to check:** follow-up after an answer (keys did not reach the card once — stopped driving when loginwindow went frontmost, maybe the owner at the Mac); Replace, Copy, Note, Share on an answer; Bullets, Grammar, Shorten, Explain, Key points, Translate; Ask AI typed. Then C, D, E, F.** Written so work can resume exactly here
