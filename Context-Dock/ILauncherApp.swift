...
class AppDelegate: NSObject, NSApplicationDelegate, NSWindowDelegate {
    ...
    /// ContentView reads this on `launcherWindowOpened` to decide whether to reset to L1.
    ...
    func toggleLauncher() {
        ...
        DispatchQueue.main.async {
            if let window = self.launcherWindow {
                if window.isVisible {
                    if self.isDockContextMode {
                        // Dock context is showing → switch to L1 (don't close, just change mode)
                        self.isDockContextMode = false
                        NotificationCenter.default.post(name: .switchToL1, object: nil)
                    } else {
                        // Already at L1 → close
                        self.hideLauncher()
                    }
                } else {
                    // Hidden → always open at L1
                    self.isDockContextMode = false
                    self.showLauncher()
                }
            }
    ...
    func showLauncher() {
        ...
        print("🔍 [AppDelegate] Detecting context from captured app...")
        detectAndStoreUserContextAsync()
        ...
        // Spotlight-style launch animation: start small + transparent, spring to full size
        let targetFrame = initialFrame
        // Start slightly smaller (scale-in feel) but stay within a pixel of final height
        // so updateWindowSize() sees no change and its guard suppresses a second resize.
        let animStartFrame = NSRect(
            x: targetFrame.midX - targetFrame.width * 0.5,
            y: targetFrame.midY - targetFrame.height * 0.5,
            width: targetFrame.width * 0.96,
            height: targetFrame.height * 0.96
        )
        window.setFrame(animStartFrame, display: false)
        window.alphaValue = 0
        ...
        // Animate in: fade + subtle scale-up (Spotlight feel, tight range so no jump)
        NSAnimationContext.runAnimationGroup { ctx in
            ctx.duration = 0.20
