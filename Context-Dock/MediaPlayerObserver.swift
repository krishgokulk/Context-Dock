...
class MediaPlayerObserver: ObservableObject {
    ...
    func togglePlayPause() {
        ...
        isPlaying.toggle()
        if isPlaying { anchorDate = Date() } else { playbackRate = 0 }

        MediaRemoteBridge.shared.sendCommand(.togglePlayPause)

        if let bid = lastBundleID, isBrowser(bid) {
            Task.detached { await self.browserToggle(bundleID: bid) }
        }

        // Confirm real state after action settles.
        scheduleFetch(delay: 0.5)
    }
    ...
    private func browserToggle(bundleID: String) async {
