...
final class AXWebReader {
    ...
    private var inFlightURLs: [pid_t: String] = [:]
    private var lastAttempts: [pid_t: RefreshAttempt] = [:]
    private let stateLock = NSLock()
    private let minimumRepeatInterval: TimeInterval = 1.8
    ...
    func cachedSnapshot(for pid: pid_t) -> PageSnapshot? {
        ...
        guard let snap = cache[pid], !snap.isStale else {
    ...
    func refresh(pid: pid_t, currentURL: String) {
        let normalizedURL = currentURL.trimmingCharacters(in: .whitespacesAndNewlines)
        let now = Date()

        stateLock.lock()
        // Skip if cache is fresh and URL hasn't changed
        if let cached = cache[pid], !cached.isStale, cached.url == normalizedURL {
            stateLock.unlock()
            return
        }
        // Coalesce duplicate refreshes for the same URL and suppress rapid repeats.
        if let inFlightURL = inFlightURLs[pid], inFlightURL == normalizedURL {
            stateLock.unlock()
            return
        }
        if let attempt = lastAttempts[pid],
           attempt.url == normalizedURL,
           now.timeIntervalSince(attempt.startedAt) < minimumRepeatInterval
        {
            stateLock.unlock()
            return
        }
        inFlightURLs[pid] = normalizedURL
        lastAttempts[pid] = RefreshAttempt(url: normalizedURL, startedAt: now)
        stateLock.unlock()

        let axApp = AXUIElementCreateApplication(pid)

        // Find AXWebArea starting from the focused window
        guard let webArea = findWebArea(axApp) else {
            finishRefresh(pid: pid, url: normalizedURL, snapshot: nil)
            return
        }
        ...
        finishRefresh(pid: pid, url: normalizedURL, snapshot: PageSnapshot(
            url: normalizedURL,
            text: text,
            title: title,
            timestamp: Date()
    ...
    func invalidate(pid: pid_t) {
        stateLock.lock()
        cache.removeValue(forKey: pid)
        inFlightURLs.removeValue(forKey: pid)
        stateLock.unlock()
    }

    private func finishRefresh(pid: pid_t, url: String, snapshot: PageSnapshot?) {
        stateLock.lock()
        defer { stateLock.unlock() }

        // Only the newest requested URL for this pid should win.
        guard inFlightURLs[pid] == url else { return }
        if let snapshot {
            cache[pid] = snapshot
        }
        inFlightURLs.removeValue(forKey: pid)
