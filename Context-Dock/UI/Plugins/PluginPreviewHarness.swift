...
/// The Developer Inspector's region: pick a manifest — the shipped examples, plus anything
/// actually installed — and see it in every host it declares.
@MainActor
struct PluginPreviewPanel: View {
    ...
    private var manifests: [PluginManifest] {
        PluginExamples.all + registry.plugins.map(\.manifest)
    }

    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            HStack {
                Label("Plugin preview", systemImage: "puzzlepiece.extension")
                    .font(.headline)
                Spacer()
            ...
            if let manifest = manifests.first(where: { $0.id == selection }) ?? manifests.first {
