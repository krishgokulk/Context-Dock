...
class AppleAppsAPI {
    ...
    func createEvent(title: String, startDate: Date, endDate: Date? = nil, notes: String? = nil, location: String? = nil) -> Bool {
    ...
    func getMusicVolume() -> Int? {
        ...
        """
        if let result = runAppleScript(script), let volume = Int(result.trimmingCharacters(in: .whitespacesAndNewlines)) {
            return volume
        }
        return nil
    }
        ...
        case "calendar-create":
            ...
            if args.count >= 3 {
                ...
                if let startDate = formatter.date(from: args[2]) {
                    ...
                    let success = createEvent(title: title, startDate: startDate, endDate: endDate, notes: notes, location: location)
                    data = ["success": success, "message": success ? "Event created successfully" : "Failed to create event"]
                } else {
