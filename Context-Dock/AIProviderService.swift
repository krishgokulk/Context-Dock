...
class AIProviderService: ObservableObject {
    ...
    func sendMessage(
        ...
        // Capture current Finder folder if Finder is the active app
    ...
    func buildContextPrompt(context: UserContext) -> String {
        ...
        if !settings.onDeviceSystemPrompt.isEmpty {
            return settings.onDeviceSystemPrompt
        }
        return prompt
    }
    ...
    private func buildContextPrompt(
        ...
        prompt += "You have access to the ILauncher API system which allows you to ACTUALLY PERFORM ACTIONS on the user's behalf. "
        prompt += "Available actions:\n"
        prompt += "- Safari PDF Export: When in Safari, you can trigger PDF export by running: `ilauncher-api safari save-pdf [filename]`\n"
        prompt += "- Safari Print: Trigger print dialog with: `ilauncher-api safari print`\n"
        prompt += "- Get current Safari page: `ilauncher-api safari current-page`\n"
        prompt += "\nWhen the user asks to save a Safari page as PDF, ACTUALLY DO IT by calling the API, don't just explain how to do it manually!\n"
        prompt += "Format your API calls in a code block like this: ```bash\nilauncher-api safari save-pdf webpage\n```\n"
        prompt += "The system will execute these commands and show the results to the user.\n"

        // Add terminal command capabilities (only for legacy text-parsing mode;
        // sendWithTools uses function-calling so skip the [TERMINAL_COMMAND:] text format)
        if !forToolUse {
            prompt += "\n" + AITerminalPrompts.terminalCapabilities
        }
    ...
    func sendOnDevice(_ message: String, context: UserContext, history: [ChatMessage] = []) async throws -> String {
    ...
    private func onDeviceSession(for contextPrompt: String) async throws -> LanguageModelSession {
        let settings = AppSettings.shared
        let systemPrompt = settings.onDeviceSystemPrompt.isEmpty ? contextPrompt : settings.onDeviceSystemPrompt
