...
class AppSettings: ObservableObject {
    ...
    @AppStorage("webExtensionsData") private var webExtensionsData: Data = Data()
    ...
    @Published var webExtensions: [WebExtension] = [] {
        didSet {
            saveWebExtensions()
        }
    }
    ...
    /// Returns Quick Actions for a given app key (placement: .quickActions or .both).
    /// Falls back to built-in defaults if the user hasn't customised them yet.
    func shortcuts(for appKey: String) -> [AppShortcut] {
        let custom = appShortcuts.filter {
            ...
            ($0.placement == .quickActions || $0.placement == .both)
        }
        if !custom.isEmpty {
            var seen = Set<String>()
            return custom.filter { seen.insert($0.name).inserted }
        }
        // Built-ins are always quick actions
        return AppShortcut.builtInDefaults.filter { $0.appKey == appKey }
    }
    ...
    func contextDockShortcuts(for appKey: String) -> [AppShortcut] {
        let custom = appShortcuts.filter {
            ($0.appKey == appKey || $0.targetAppKeys.contains(appKey)) &&
            ($0.placement == .contextDock || $0.placement == .both)
        }
        var seen = Set<String>()
        return custom.filter { seen.insert($0.name).inserted }
    }
    ...
    func addShortcut(_ shortcut: AppShortcut) {
        // On first edit, seed the full defaults so the user's list contains all entries
        if appShortcuts.isEmpty {
            appShortcuts = AppShortcut.builtInDefaults
        }
    ...
    func updateShortcut(_ shortcut: AppShortcut) {
        // Seed built-in defaults first so built-in shortcuts become editable
        if appShortcuts.isEmpty {
            appShortcuts = AppShortcut.builtInDefaults
        }
        if let idx = appShortcuts.firstIndex(where: { $0.id == shortcut.id }) {
            ...
            // Shortcut was a built-in default not yet in the custom list — promote it
            appShortcuts.append(shortcut)
        }
    }
    ...
    private func loadWebExtensions() {
        guard !webExtensionsData.isEmpty else { return }

        do {
            let decoder = JSONDecoder()
            webExtensions = try decoder.decode([WebExtension].self, from: webExtensionsData)
        } catch {
            print("⚠️ Failed to decode web extensions: \(error)")
        }
    }

    private func saveWebExtensions() {
        do {
            let encoder = JSONEncoder()
            webExtensionsData = try encoder.encode(webExtensions)
        } catch {
            print("⚠️ Failed to encode web extensions: \(error)")
        }
    }
