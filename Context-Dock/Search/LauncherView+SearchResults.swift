...
extension LauncherView {
    ...
    @ViewBuilder
    /// One dock pill. The drawing lives in `UnifiedPillButton`, which takes values rather than
    /// launcher state; everything it needs is resolved here.
    func unifiedPillButton(
        ...
        UnifiedPillButton(
            ...
            isCompact: isCompact,
            accent: accentColor(for: pill.accentColorName),
            isFocused: l2.focusedPillIndex == index,
            showsBadge: l2.focusedPillIndex == nil,
            onActivate: {
                executeDockPill(pill)
                searchState.query = ""
                l2.focusedPillIndex = nil
