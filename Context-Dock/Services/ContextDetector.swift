...
class ContextDetector {
    ...
    /// Prefers live data pushed by the Safari Web Extension (richer, no permission
    /// prompts); falls back to AppleScript when the extension is not installed/enabled.
    func getSafariContext() -> (url: String, title: String)? {
        // Extension path — instant, no automation prompt, also carries page text / selection
        if let ctx = SafariBrowserBridge.shared.safariContextIfFresh() {
            return ctx
        }
    ...
    func getSafariSelectedText() -> String? {
        SafariBrowserBridge.shared.selectedTextIfRecent()
    }

    /// Returns the full page context block for the active Safari tab, suitable
    /// for pasting directly into an AI prompt. Only available via extension.
    func getSafariPageContextForAI() -> String? {
        guard SafariBrowserBridge.shared.isFresh else { return nil }
        let block = SafariBrowserBridge.shared.aiContextBlock()
