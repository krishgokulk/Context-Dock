...
struct AXTriggerRule: Codable, Identifiable, Equatable {
    ...
    static let legacySelectionScopeBuiltInNames: Set<String> = [
        "Selection — Ask AI",
        "Selection — Document Brief",
        "Selection — OCR Image",
        "Selection — Compress File",
        "Selection — Link Summary",
    ]
    ...
    static let selectionScopeBuiltInExamples: [AXTriggerRule] = [
        ...
        AXTriggerRule(
            ...
            pills: [
                AXRulePill(
                    ...
                    accentColor: "blue",
                    actionType: .shellCommand,
                    actionValue: "printf '%s' '{selectedText}' | pbcopy"
                ),
            ],
            priority: 19
        ),
        ...
        AXTriggerRule(
            ...
            pills: [
                AXRulePill(
                    ...
                    accentColor: "teal",
                    actionType: .shellCommand,
                    actionValue: "printf '%s' '{file}' | pbcopy"
                ),
            ],
            priority: 18
        ),
        ...
        AXTriggerRule(
            ...
            pills: [
                AXRulePill(
                    ...
                    accentColor: "purple",
                    actionType: .shellCommand,
                    actionValue: "printf '%s' 'Describe this image: {file}' | pbcopy"
                ),
            ],
            priority: 17
        ),
        AXTriggerRule(
            ...
            isEnabled: true,
            conditions: [
                AXTriggerCondition(field: .filePath, op: .matchesRegex, value: "(?i)\\.(png|jpg|jpeg|heic|webp|tif|tiff)$"),
            ],
            conditionLogic: .all,
            pills: [
                AXRulePill(
                    ...
                    accentColor: "blue",
                    actionType: .shellCommand,
                    actionValue: "sips -s format jpeg '{file}' --out '{file}.jpg'"
                ),
            ],
            priority: 16
        ),
        AXTriggerRule(
            ...
            isEnabled: true,
            conditions: [
                AXTriggerCondition(field: .filePath, op: .matchesRegex, value: "(?i)\\.(png|jpg|jpeg|heic|tif|tiff)$"),
            ],
            conditionLogic: .all,
            pills: [
                AXRulePill(
                    ...
                    accentColor: "orange",
                    actionType: .shellCommand,
                    actionValue: "sips -s format jpeg -s formatOptions 72 '{file}' --out '{file}.compressed.jpg'"
                ),
            ],
            priority: 16
        ),
...
final class AXTriggerRuleEngine {
    ...
    func testAgainstCurrentScreen(rule: AXTriggerRule) -> [(field: String, liveValue: String, matched: Bool)] {
        ...
        let ctx = AXContextReader.shared.current
        let clipboard = NSPasteboard.general.string(forType: .string)
        return rule.conditions.map { cond in
            let live = value(of: cond.field, context: ctx, clipboard: clipboard)
            let matched = apply(op: cond.op, to: live, against: cond.value)
            return (field: cond.field.rawValue, liveValue: live, matched: matched)
        }
    }
...
// MARK: - PillRow UI additions (assumed location for editing)

// Below is the modified snippet for the Picker where action types are listed:
//
// Picker("Action Type", selection: $pill.actionType) {
...
//     Text("Menu Item").tag(AppShortcut.ActionType.menuItem)  // Added new option
// }
//
// Also, in AXRulePill.actionPlaceholder switch, add:
//
// case .menuItem:    return "File > New Tab"
//
// And in the UI where the actionValue TextField is shown:
//
// if pill.actionType == .menuItem {
//     VStack(alignment: .leading, spacing: 6) {
//         HStack(spacing: 8) {
//             TextField("File > New Tab", text: $pill.actionValue)
//                 .textFieldStyle(.roundedBorder)
//                 .font(.system(.caption, design: .monospaced))
//             Menu("Pick from current app…") {
//                 let items = AXContextReader.shared.current.menuItems
//                 ForEach(items.prefix(200), id: \.id) { info in
//                     Button("\(info.fullPath)") { pill.actionValue = info.fullPath }
//                 }
//             }
//             .menuStyle(.borderlessButton)
//             .controlSize(.small)
//         }
//         Text("Tip: Use the exact menu path like ‘File > New Tab’. The list above is live from the frontmost app.")
